...
export function Simulador({
  ...
  /** A sessão já queimou uma senha de liberação? Vem do servidor. */
  descontoJaLiberado: boolean;
}) {
  const router = useRouter();

  // Vendedor emite em nome próprio; admin/gestor escolhem depois.
  const consultorInicial = usuario.papel === "vendedor" ? usuario.nome : "";
  ...
  return (
    <div className="min-h-dvh pb-28 lg:pb-0">
      ...
      <header className="sticky top-0 z-30 border-b border-marca-borda bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Botao variante="sutil" onClick={sair} title="Sair">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        {etapa === 3 ? (
          <>
            <EtapaResultado r={resultado} config={config} />
            ...
            <div>
              {etapa === 1 ? (
                ...
                <EtapaUnidades
                  ...
                  onPedirLiberacao={() => setModalSenha(true)}
                  config={config}
                />
              )}
      ...
      {/* Senha de uso único para liberar o desconto. */}
      <ModalSenhaDesconto
        aberto={modalSenha}
        onFechar={() => setModalSenha(false)}
        onLiberado={() => setLiberado(true)}
      />
    </div>
  );
}
